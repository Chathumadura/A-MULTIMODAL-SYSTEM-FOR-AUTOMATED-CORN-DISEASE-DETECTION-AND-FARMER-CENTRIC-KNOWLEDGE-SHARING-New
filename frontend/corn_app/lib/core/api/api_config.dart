...
//   RunMode.production  → https://a-multimodal-system-for-automated-corn-71fp.onrender.com
//
// ── Developer quick-start ─────────────────────────────────────────────────
//
//  1. Default (Android emulator or real device) → always hits Render production:
//       flutter run
//
//  2. Real device on the same WiFi (local backend):
...
//
//  3. Local emulator against a local backend (dev only):
//       flutter run --dart-define=RUN_MODE=emulator
//
...
const String _productionUrl = 'https://a-multimodal-system-for-automated-corn-71fp.onrender.com';
...
class ApiConfig {
  ...
  ///  3. Debug on Android → [RunMode.production]  (hits Render by default)
  ...
  static RunMode get runMode {
    ...
    // 3. Debug + Android emulator: default to production (Render API).
    //    Override with --dart-define=RUN_MODE=emulator for local testing.
    if (!kIsWeb && Platform.isAndroid) return RunMode.production;
