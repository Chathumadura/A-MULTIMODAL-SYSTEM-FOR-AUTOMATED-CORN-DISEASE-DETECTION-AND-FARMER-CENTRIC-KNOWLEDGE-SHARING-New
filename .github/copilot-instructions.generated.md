...
- Yield ML backend: `yield-prediction-backend/` — FastAPI service that loads `models/corn_yield_model.tflite`, predicts and returns SHAP explanations.
...
- Model expectations: pipeline expects ~23 features. The helper `build_full_row_from_simple()` (in `yield-prediction-backend/main.py`) fills many defaults — changing these defaults will change predictions. Keep defaults in sync with training data and `models/corn_yield_model.tflite`.
...
- Ensure `models/corn_yield_model.tflite` is present in `yield-prediction-backend/` and built with a compatible sklearn version.
